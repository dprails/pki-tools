...
## stolen from https://raw.githubusercontent.com/confluentinc/confluent-platform-security-tools/master/kafka-generate-ssl.sh
...
if [ "$generate_trust_store" == "y" ]; then
  ...
  echo "autogenerating CA password"
  ...
  openssl req -new -x509 -keyout $TRUSTSTORE_WORKING_DIRECTORY/ca-key \
    -out $TRUSTSTORE_WORKING_DIRECTORY/ca-cert -days $VALIDITY_IN_DAYS -passout "pass:$ca_password"
  ...
  echo " - $TRUSTSTORE_WORKING_DIRECTORY/ca_password.txt - holds the password for the CA key"
  ...
  echo "autogenerating truststore password"
  ...
  keytool -keystore $TRUSTSTORE_WORKING_DIRECTORY/$DEFAULT_TRUSTSTORE_FILENAME \
    -alias CARoot -import -file $TRUSTSTORE_WORKING_DIRECTORY/ca-cert -storepass $truststore_password
...
echo "autogenerating keystore password"
mkdir $KEYSTORE_WORKING_DIRECTORY
keystore_password="$(cat /dev/urandom | tr -dc 'a-zA-Z0-9' | fold -w 12 | head -n 1)"
echo $keystore_password > $KEYSTORE_WORKING_DIRECTORY/keystore_password.txt
...
keytool -keystore $KEYSTORE_WORKING_DIRECTORY/$KEYSTORE_FILENAME \
  -alias localhost -validity $VALIDITY_IN_DAYS -genkey -keyalg RSA -storepass $keystore_password
...
keytool -keystore $trust_store_file -export -alias CARoot -rfc -file $CA_CERT_FILE -storepass $truststore_password
...
keytool -keystore $KEYSTORE_WORKING_DIRECTORY/$KEYSTORE_FILENAME -alias localhost \
  -certreq -file $KEYSTORE_SIGN_REQUEST -storepass $keystore_password
...
openssl x509 -req -CA $CA_CERT_FILE -CAkey $trust_store_private_key_file \
  ...
  -days $VALIDITY_IN_DAYS -CAcreateserial -passin "pass:$truststore_password"
...
keytool -keystore $KEYSTORE_WORKING_DIRECTORY/$KEYSTORE_FILENAME -alias CARoot \
  -import -file $CA_CERT_FILE -storepass $keystore_password
