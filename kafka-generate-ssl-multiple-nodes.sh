...
while [ $counter -lt $limit ]; do
  ...
  KEYSTORE_DIR=$KEYSTORE_WORKING_DIRECTORY$counter
  mkdir $KEYSTORE_DIR
  ...
  echo "     NOTE: make the CN the FQDN. Some operating systems call the CN prompt 'first / last name'"
  ...
  keytool -keystore $KEYSTORE_DIR/$KEYSTORE_FILENAME \
    -alias localhost -validity $VALIDITY_IN_DAYS -genkey -keyalg RSA

  echo
  echo "'$KEYSTORE_DIR/$KEYSTORE_FILENAME' now contains a key pair and a"
  ...
  keytool -keystore $KEYSTORE_DIR/$KEYSTORE_FILENAME -alias localhost \
  ...
  keytool -keystore $KEYSTORE_DIR/$KEYSTORE_FILENAME -alias CARoot \
  ...
  echo
  echo "You will be prompted for the keystore's password."
  keytool -keystore $KEYSTORE_DIR/$KEYSTORE_FILENAME -alias localhost -import \
